...
func introduction(name : String, home : String, age: Int){
    print("if \"\(name)\", \"\(home)\" and \(age) were passed into a function, it might print \"\(name), \(age), is from California\".")
}
          
          introduction(name: "Jenny", home: "California", age: 32)

//:  Write a function called `almostAddition` that takes two `Int` arguments. The first argument should not require an argument label. The function should add the two arguments together, subtract 2, then print the result. Call the function and observe the printout.
func almostAddition(_ one: Int , and  two : Int) {
    let result = one+two - 2;
    
    print(result)
}

almostAddition(1, and: 34)
